Ext.define('GbZh.geoext.PrintPage', {
    mixins: {
...
//            this.setScale(this.printProvider.capabilities.scales[0]);
        } else {
...
/*         if (fitTo instanceof GeoExt.MapPanel) {
        ...
        this.printProvider.scales.each(function (rec) {
			//LOG console.log(rec.data.value);
 ...
 //--           f.layer && f.layer.drawFeature(f);
            ...
            if (modified) {
				this.fireEvent("change", this, mods);
			}
        }
    },
     ...
     */
    calculatePageBounds: function (scale, mapunits) {
...
//--        var size = this.printProvider.layout.get("size");
		var height = this.printProvider.layout.data.map.height;
		var width = this.printProvider.layout.data.map.width;
        var units = mapunits ||
 ...
 */
		var w = width / 72 / unitsRatio * s / 2;
        var h = height / 72 / unitsRatio * s / 2;

		//LOG console.log("w/h/scale: " + w + " " + h +  " " + s);
