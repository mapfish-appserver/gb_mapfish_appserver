Ext.define('GbZh.geoext.PrintPageField', {
	mixins: {
	...
	init: function (target) {
		...
		var eventName = target instanceof Ext.form.ComboBox ? "select" : target instanceof Ext.form.Checkbox ? "check" : "change";
		onCfg[eventName] = this.onFieldChange;
		target.on(onCfg);
		//HACK
		this.printPage = Ext.getCmp('mapcomponent').plugins[0].page;
		...
		Ext.util.Observable.capture(this, function (e, params) {
			//LOG console.log('PrintPageField-event: ' + e);
	...
	onFieldChange: function (field, record) {
		...
		this.updating = true;
		if (field.store === printProvider.scales || field.id === "scale") {
			this.printPage.setScale(record);
//		} else if (field.name === "rotation") {
		} else if (field.id === "rotation") {
			if (!isNaN(value)) {
				this.printPage.setRotation(value);
			}
		} else {
